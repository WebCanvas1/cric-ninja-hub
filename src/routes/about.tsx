...
export const Route = createFileRoute("/about")({
  head: () => ({
    meta: [
      { title: "About CRIC NINJA — The Craft" },
      { name: "description", content: "The story behind CRIC NINJA — master craftsmen, premium willow, and cricket bats built for champions." },
    ],
  }),
  component: About,
});
    ...
    <SiteLayout>
      <section className="bg-hero">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-20 sm:px-6 md:grid-cols-2 md:items-center">
          <div>
            <div className="text-xs font-bold uppercase tracking-[0.3em] text-primary">Our Story</div>
            <h1 className="display mt-2 text-5xl font-bold uppercase leading-tight tracking-wider sm:text-6xl">{content.about.title}</h1>
          ...
          {content.whyChooseUs.map((f, i) => {
            ...
            return (
              <div key={i} className="rounded-md border border-border bg-card p-6">
                ...
                <h3 className="display mt-4 text-lg font-bold uppercase">{f.title}</h3>
